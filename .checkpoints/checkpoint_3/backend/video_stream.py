...
class VideoStreamProcessor:
    ...
    def __init__(self, detector=None, max_fps: int = 25):
        ...
        self.colors = {
            ...
            "Mask": (0, 140, 100), "Safety Shoes": (0, 120, 80), "NO Helmet": (0, 0, 255),
            "NO Vest": (0, 60, 255), "NO Goggles": (80, 50, 255), "NO Mask": (120, 100, 255),
            "NO Safety Shoes": (140, 120, 255), "face": (0, 255, 255), "default": (128, 128, 128),
        }
    ...
    def _capture_loop(self, video_source):
        ...
        # Latency optimization: set buffer size to 1
        ...
        print(f"Stream started. Resolution: 640x360, Source: {video_source}")
    ...
    def _annotate_cv2(self, frame: np.ndarray, detections: dict) -> np.ndarray:
        """Fast OpenCV annotation."""
        img = frame.copy()
        try:
            for det in detections.get("ppe", []):
                x1, y1, x2, y2 = map(int, det["bbox"])
                color = self.colors.get(det["label"], (128, 128, 128))
                cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
                cv2.putText(img, det["label"], (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    ...
    def _inference_loop(self):
        """Runs ML inference on a separate thread to prevent blocking video capture."""
        while not self.stop_event.is_set():
            ...
            if frame_to_process is not None and self.detector:
                try:
                    ...
                    print(f"DEBUG [VideoStream Inference]: {e}")
            else:
                time.sleep(0.01)
    ...
    def get_frame_nowait(self) -> Optional[dict]:
        """Get the latest frame if it's new."""
        with self._buffer_lock:
